...
class UserController extends Controller
{
    ...
    public function loadRoles(Request $request){
        ...
        $userRoles = AgentRoleUser::where(['user_id'=>$request->user_id])->pluck('role_id')->toArray();

        $roles = Role::where(['is_view_agent_panel'=>1,'is_active'=>1])->get();
        return view("agent::ajax-load.selected-role",[
    ...
    public function loadDistrictByDivision(Request $request){
        $division_ids = array();
        ...
        foreach($districts as $district){
            echo '<option value="'.$district->id.":".$district->division_id.'">'.$district->name.'</option>';
        }
    }
    ...
    public function loadUpazilaByDistrict(Request $request){
        ...
        foreach($upazilas as $upazila){
            echo '<option value="'.$upazila->id.":".$upazila->district_id.'">'.$upazila->name.'</option>';
        }
    }
    ...
    public function index($module){
        ...
        ->whereIn('flag',getAgentConsultantUserFlag())
        ->orderBy('id','desc')
        ->paginate(100);
        ...
        $roles = Role::whereIn('flag',[19,24])->get();
        return view('agent::users.index',compact('users','roles','module','search','role_id','filter_by','last_updated'));
    }



    public function create($module){
        $roles = Role::where(['flag'=>24])->get(['id','name']);
    ...
    public function store(Request $request,$module){
        ...
        try{
             ...
             return redirect()->route('agent.users.index',$module);
    ...
    public function editPermission($mod,$user_id)
    {
        ...
        return view('agent::users.edit-permission',[
            'modules'=> $modules,
            'allPermissions'=>$user,
            'moduleIds'=>$moduleids,
    ...
    public function editPermissionSubmit(Request $request,$mod,$user_id){
        ...
        try{
            ...
            return redirect()->route('agent.users.index',$mod);
        }catch (\Exception $exception){
            DB::rollback();
            Session::flash('error',$exception->getMessage());
