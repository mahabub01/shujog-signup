...
class LogController extends Controller
{
    public function index($module)
    {
        ...
        if ($flag == 22) {
            $user_id = User::whereIn('flag', [22, 28])->pluck('id')->toArray();
        ...
        return view("agent::nmanagers.log.log", [
    ...
    public function filter(Request $request, $module)
    {
        ...
        if ($flag == 22) {
            ...
            // search by name
            if($request->search != ""){
                $user_id = User::whereIn('flag', [22, 28])->where('name','like','%'.$request->search.'%')->pluck('id')->toArray();
