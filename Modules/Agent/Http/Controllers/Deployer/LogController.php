...
class LogController extends Controller
{
    public function index($module)
    {
        ...
        if ($flag == 21) {
            $user_id = User::whereIn('flag', [21, 26])->pluck('id')->toArray();
        ...
        return view("agent::deployers.log.log", [
    ...
    public function filter(Request $request, $module)
    {
        ...
        if ($flag == 21) {
            ...
            // search by name
            if($request->search != ""){
                $user_id = User::whereIn('flag', [21, 26])->where('name','like','%'.$request->search.'%')->pluck('id')->toArray();
