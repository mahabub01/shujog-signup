...
use Modules\Agent\Exports\SelectableExport;
...
use Modules\Agent\Exports\AllStakeholderExport;
...
class StakeholderController extends Controller
{
    ...
    public function index($module,Request $request)
    {
        ...
        if(isset($request->role)){
            ...
            $stakeholders = User::with(['stakeholder','stakeholderEvaluation','stakeholderCommnet','stakeholderCommnetForTrainer','signupReference','stakeholderCommnetForDeployer','stakeholderCommnetForNmanager','assignProject'=>function($q){
                $q->select('stakeholder_id','project_id');
            },'assignProject.project'])
            ->where(['spatie_role_id'=>$request->role])
            ->whereHas('stakeholder',function($query){
                $query->where('consultant_status',3)->whereIn('trainer_status',[1,2,3]);
            })
            ->orderBy(Stakeholder::select('trainer_status')->whereColumn('agent_stakeholders.user_id','sujog_users.id'))
            ->paginate(100);
        ...
        return view("agent::trainers.stakeholders.index",[
    ...
    public function filter(Request $request,$module)
    {
        ...
        if(isset($request->role)){
            ...
            $stakeholders_query = User::with(['stakeholder','stakeholderEvaluation','stakeholderCommnet','stakeholderCommnetForTrainer','signupReference','stakeholderCommnetForDeployer','stakeholderCommnetForNmanager','assignProject'=>function($q){
                $q->select('stakeholder_id','project_id');
            },'assignProject.project'])
            ->where(['spatie_role_id'=>$request->role]);
            ...
            $data =  $stakeholders_query->orderBy(Stakeholder::select('trainer_status')->whereColumn('agent_stakeholders.user_id','sujog_users.id'))
            ->paginate(100);
            ...
            $first_active_tab = $selectRoles->first();
            $stakeholders_query = User::with('stakeholder','stakeholderEvaluation','stakeholderCommnet','stakeholderCommnetForTrainer')
            ->where(['spatie_role_id'=>$first_active_tab->role_id]);
            ...
            $data =  $stakeholders_query->orderBy(Stakeholder::select('trainer_status')->whereColumn('agent_stakeholders.user_id','sujog_users.id'))->paginate(100);
    ...
    public function allExport(Request $request,$module){
        ...
        return Excel::download(new TrainerAllStakeholderExport($request), time().'-all-stackholders.xlsx');
    }
    ...
    public function selectable(Request $request){
        ...
        return Excel::download(new TrainerSelectableExport($request), time().'_stakeholders.xlsx');
    ...
    *********************************/
        ...
        return view("agent::trainers.stakeholders.details",[
            'user'=>$user,
            'module'=>$module,
    ...
    public function commentView($module,$user_id)
    {
        ...
        return view("agent::trainers.stakeholders.comments.comment",[
            'module'=>$module,
            'user_id'=>$user_id,
    ...
    public function consultantCommentSubmit(Request $request,$module,$user_id)
    {
        ...
        try{
            ...
            return redirect($module.'/stakeholders/trainer-comments/'.$user_id);
