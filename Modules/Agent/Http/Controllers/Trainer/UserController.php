...
class UserController extends Controller
{
    ...
    public function index($module){
        ...
        ->whereIn('flag',getAgentTrainerUserFlag())
        ->orderBy('id','desc')
        ->paginate(100);
        ...
        $roles = Role::whereIn('flag',[20,25])->get();
        return view('agent::trainers.users.index',compact('users','roles','module','search','role_id','filter_by','last_updated'));
    }



    public function create($module){
        $roles = Role::where(['flag'=>25])->get(['id','name']);

        return view('agent::trainers.users.create',[
             'module'=>$module,
             'roles'=>$roles
        ]);
    }
    ...
    public function store(Request $request,$module){
        ...
        try{
             ...
             return redirect()->route('agent.trainer-users.index',$module);
    ...
    public function editPermission($mod,$user_id)
    {
        ...
        return view('agent::trainers.users.edit-permission',[
            'modules'=> $modules,
            'allPermissions'=>$user,
            'moduleIds'=>$moduleids,
    ...
    public function editPermissionSubmit(Request $request,$mod,$user_id){
        ...
        try{
            ...
            return redirect()->route('agent.trainer-users.index',$mod);
        }catch (\Exception $exception){
            DB::rollback();
            Session::flash('error',$exception->getMessage());
    ...
    public function filter($module,Request $request){
       ...
       $roles = Role::whereIn('flag',getAgentTrainerUserFlag())
       ->where(['is_active'=>1])
       ->get(['id','name','flag']);
    ...
    public function edit($module,$user_id)
    {
        ...
        $roles = Role::where(['flag'=>25])->get(['id','name']);
        return view('agent::trainers.users.edit',[
             'module'=>$module,
             'roles'=>$roles,
             'user'=>$user
        ]);
