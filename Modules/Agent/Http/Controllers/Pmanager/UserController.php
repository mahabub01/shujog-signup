...
class UserController extends Controller
{
    ...
    public function index($module){
        $users = User::with('spatieRole','assignProMngProject','assignProMngProject.project')
        ->whereIn('flag',getAgentPmanagerUserFlag())
        ->orderBy('id','desc')
        ->paginate(100);
        ...
        $roles = Role::whereIn('flag',getAgentPmanagerUserFlag())->get();
        return view('agent::pmanagers.users.index',compact('users','roles','module','search','role_id','filter_by','last_updated'));
    }



    public function create($module){
        $roles = Role::where(['flag'=>27])->get(['id','name']);

        return view('agent::pmanagers.users.create',[
             'module'=>$module,
             'roles'=>$roles
        ]);
    }
    ...
    public function store(Request $request,$module){
        ...
        try{
             ...
             return redirect()->route('agent.pmanager-users.index',$module);
    ...
    public function editPermission($mod,$user_id)
    {
        ...
        return view('agent::pmanagers.users.edit-permission',[
            'modules'=> $modules,
            'allPermissions'=>$user,
            'moduleIds'=>$moduleids,
    ...
    public function editPermissionSubmit(Request $request,$mod,$user_id){
        ...
        try{
            ...
            return redirect()->route('agent.pmanager-users.index',$mod);
        }catch (\Exception $exception){
            DB::rollback();
            Session::flash('error',$exception->getMessage());
    ...
    public function assignProject($module,Request $request)
    {
        $this->validate($request,[
            'projects'=>'required'
        ]);
        ...
        try{
            ...
            //Remove old Data
            AgentProMngAssignProject::where(['user_id'=>$request->user_id])->delete();

            foreach($request->projects as $project){
                AgentProMngAssignProject::create([
                    'user_id'=>$request->user_id,
                    'project_id'=>$project
                ]);
            }

            Session::flash('success',"Project Assigned Successfully");
            return redirect()->route('agent.pmanager-users.index',$module);
    ...
    public function filter($module,Request $request){
       ...
       $query = User::with('spatieRole','assignProMngProject','assignProMngProject.project')
       ->whereIn('flag',getAgentPmanagerUserFlag());
       ...
       $roles = Role::whereIn('flag',getAgentPmanagerUserFlag())->where(['is_active'=>1])->get();
    ...
    public function edit($module,$user_id)
    {
        ...
        $roles = Role::where(['flag'=>27])->get(['id','name']);
        return view('agent::pmanagers.users.edit',[
             'module'=>$module,
             'roles'=>$roles,
             'user'=>$user
        ]);
