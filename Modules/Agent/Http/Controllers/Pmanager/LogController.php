...
class LogController extends Controller
{
    public function index($module)
    {
        ...
        if ($flag == 23) {
            $user_id = User::whereIn('flag', [23, 27])->pluck('id')->toArray();
        ...
        return view("agent::pmanagers.log.log", [
    ...
    public function filter(Request $request, $module)
    {
        ...
        if ($flag == 23) {
            ...
            // search by name
            if($request->search != ""){
                $user_id = User::whereIn('flag', [23, 27])->where('name','like','%'.$request->search.'%')->pluck('id')->toArray();
