...
class TrainerAllRejectStkhExport implements FromView
{
    public $request;
    ...
    public function view(): View
    {
        ...
        if($this->request->status != ""){
            $stakeholders_query->whereHas('stakeholder',function($query){
                $query->where('trainer_status',$this->request->status)->where('consultant_status','3');
            });
        }else{
            $stakeholders_query->whereHas('stakeholder',function($query){
                $query->whereIn('trainer_status',[4])->where('consultant_status','3');
            });
        }
