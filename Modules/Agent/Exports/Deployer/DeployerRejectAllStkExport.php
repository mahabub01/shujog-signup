...
namespace Modules\Agent\Exports\Deployer;
use App\Models\User;
...
class DeployerRejectAllStkExport implements FromView{
    ...
    public function view(): View
    {
        ...
        if($this->request->status != ""){
            $stakeholders_query->whereHas('stakeholder',function($query){
                ...
                ->where('deployer_status',$this->request->status);
            });
        }else{
            $stakeholders_query->whereHas('stakeholder',function($query){
        ...
        $data =  $stakeholders_query->orderBy(Stakeholder::select('deployer_status')->whereColumn('agent_stakeholders.user_id','sujog_users.id'))->get();

        $evalu_questions = EvaluationQuestion::get();

        return view('agent::export.deployers.all-stakeholder',[
            'stakeholders'=>$data,
