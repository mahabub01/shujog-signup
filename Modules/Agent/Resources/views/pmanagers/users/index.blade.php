...
@component('core::inc.filter')
    @slot('filter_route')
    {{ Form::open(['route'=>['agent.pmanager-users.filter',$module],'method'=>'GET']) }}
    @endslot
                ...
                <div class="container-fluid">
                  ...
                  <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                      ...
                      @auth_access('agent-admin-pmanager-user-create')
                      <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="{{ route('agent.pmanager-users.create',$module) }}"><i class="fas fa-plus"></i> New</a>
                      </li>
                      @end_auth_access
                ...
                <div class="row">
                    <div class="col-md-12 d-flex">
                        ...
                        <ul class="ms-auto d-flex">
                            ...
                            <li><a href="{{ route('agent.pmanager-users.index',$module) }}"><button type="button" class="icon_btn"><i class="fas fa-redo"></i></button></a></li>
                            {{-- <li><button class="icon_btn"><i class="fas fa-chart-pie"></i></button></li> --}}
                            @auth_access('agent-admin-pmanager-user-filter')
                        ...
                        <table class="table">
                            ...
                            @foreach ($users as $k => $user)
                              <tr>
                                ...
                                <td class="text-color">
                                    @if(!is_null($user->assignProMngProject))
                                        @foreach($user->assignProMngProject as $assignPro)
                                            <span class="role-bg-color">{{ $assignPro->project->name }}, </span>
                                        @endforeach
                                    @else
                                        N/A
                                    @endif
                                ...
                                <td class="col-serial">
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-lg-end table-dropdown">
                                          ...
                                          @auth_access('agent-admin-pmanager-user-assign-project')
                                          <li><a href="javascript:void(0)" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="{{ $user->id }}"><i class="fas fa-user-tag"></i> Assign Projects</a></li>
                                          @end_auth_access

                                          @auth_access('agent-admin-pmanager-user-permission-edit')
                                          <li><a href="{{ url($module.'/pmanager-ag-edit-permission/'.$user->id) }}" class="dropdown-item"><i class="fas fa-users-cog"></i> Edit Permission</a></li>
                                          @end_auth_access


                                          @auth_access('agent-admin-pmanager-user-edit')
                                          <li><a href="{{ url($module.'/pmanager-users/'.$user->id.'/edit') }}" class="dropdown-item"><i class="fas fa-edit"></i> Edit</a></li>
                                          @end_auth_access


                                          @auth_access('agent-admin-pmanager-user-activation')
                                          @if($user->is_active == 0)
                                            <li><a href="{{ url($module.'/pmanager-users-activation/active/'.$user->id) }}" class="dropdown-item"><i class="fas fa-eye"></i> Active</a></li>
                                          @else
                                            <li><a href="{{ url($module.'/pmanager-users-activation/deactive/'.$user->id) }}" class="dropdown-item"><i class="fas fa-eye-slash"></i> De-Active</a></li>
                                          @endif
                                          @end_auth_access


                                          @auth_access('agent-admin-pmanager-user-delete')
                                        <li class="padding-left-6">
                                            {{ Form::open(['route'=>['agent.pmanager-users.destroy',[$module,$user->id]],'method'=>'DELETE','style'=>'display:inline']) }}
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    {{ Form::open(['route'=>['agent.pmanager-users.assignProject',$module],'method'=>'POST']) }}
