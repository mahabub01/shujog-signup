...
@section('title','Reject Stakeholders')
@section('content')
...
@component('core::inc.filter')
    @slot('filter_route')
    {{Form::open(['route'=>['agent.nmg-rej-stkholder.filter',$module],'method'=>'GET'])}}
    @endslot
        ...
        <div class="container-fluid  table-design-container">
            <ul class="nav nav-tabs">
                @foreach ($selectRoles as $sel_roles)
                  <li class="nav-item">
                    <a class="nav-link @if($first_active_tab->id == $sel_roles->id) active @endif" href="{{ url($module.'/nmg-reject-stkholders/?role='.$sel_roles->role_id) }}" style="font-weight: bolder;color:#495057;font-size:15px;">{{ $sel_roles->role->name }}</a>
                  </li>
                @endforeach
            </ul>
            ...
            <div class="card" style="border-radius: 0px 5px 5px 5px;border-top:none">
                    ...
                    <div class="container-fluid">
                      ...
                      <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                          ...
                          @auth_access('agent-nmg-reject-stkholder-single-export')
                          <li class="nav-item">
                            ...
                            <div class="d-flex" style="margin-bottom: 20px;">
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-lg-end">
                                      @auth_access('agent-nmg-reject-stkholder-all-export')
                                        <li><a href="{{ url($module.'/nmg-reject-stkholders-all-export?role='.$first_active_tab->role_id.'&search='.$search.'&start_date='.$start_date.'&end_date='.$end_date.'&division_id='.$division_id.'&district_id='.$district_id.'&upazila_id='.$upazila_id.'&status='.$status.'&reference_id='.$reference_id) }}" class="dropdown-item"><i class="far fa-file-excel"></i> All Export</a></li>
                                      @end_auth_access
                                    </ul>
                                  </div>


                                <a href="{{ url($module.'/nmg-reject-stkholders/?role='.$first_active_tab->role_id) }}"><button type="button" class="icon_btn"><i class="fas fa-redo"></i></button></a>
                                @auth_access('agent-nmg-reject-stkholder-filter')
                ...
                {{Form::open(['route'=>['agent.nmg-rej-stkholder.export',$module],'method'=>'POST','id'=>'exportForm'])}}
                                    ...
                                    <td class="col-serial">
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu table-dropdown">
                                              @auth_access('agent-nmg-reject-stkholder-details')
                                              <li><a href="{{ url($module.'/nmg-reject-stkholders/'.$user->id) }}" class="dropdown-item"><i class="fas fa-eye"></i> Details</a></li>
                                              @end_auth_access

                                              @auth_access('agent-nmg-reject-stkholder-cmnt-view')
                                              <li><a href="{{ url($module.'/nmg-reject-stkholders/comments/'.$user->id) }}" class="dropdown-item"><i class="far fa-comments"></i> Comment</a></li>
                                              @end_auth_access
