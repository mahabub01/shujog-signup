...
@component('core::inc.filter')
    @slot('filter_route')
    {{ Form::open(['route'=>['agent.users.filter',$module],'method'=>'GET']) }}
    @endslot
            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
                <div class="container-fluid">
                  ...
                  <div class="collapse navbar-collapse driction" id="navbarSupportedContent">
                    <div class="row">
                        <p style="margin-top: 5px;"><a href="{{ url('agent/load-component') }}" class="rev-underline-subtitle">{{ getPanelPageTitle(auth()->user()->flag) }}</a></p>
                  ...
                  <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                      ...
                      @auth_access('agent-admin-cons-user-create')
                      <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="{{ route('agent.users.create',$module) }}"><i class="fas fa-plus"></i> New</a>
                      </li>
                      @end_auth_access
                ...
                <div class="row">
                    <div class="col-md-12 d-flex">
                        ...
                        <ul class="ms-auto d-flex">
                            ...
                            <li><a href="{{ route('agent.users.index',$module) }}"><button type="button" class="icon_btn"><i class="fas fa-redo"></i></button></a></li>
                            {{-- <li><button class="icon_btn"><i class="fas fa-chart-pie"></i></button></li> --}}
                            @auth_access('agent-admin-cons-user-filter')
                                ...
                                <td class="col-serial">
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-lg-end table-dropdown">
                                          @auth_access('agent-admin-cons-user-assign-stkholder-role')
                                          ...
                                          @auth_access('agent-admin-cons-user-permission-edit')
                                          <li><a href="{{ url($module.'/ag-edit-permission/'.$user->id) }}" class="dropdown-item"><i class="fas fa-users-cog"></i> Edit Permission</a></li>
                                          @end_auth_access


                                          @auth_access('agent-admin-cons-user-edit')
                                          <li><a href="{{ url($module.'/users/'.$user->id.'/edit') }}" class="dropdown-item"><i class="fas fa-edit"></i> Edit</a></li>
                                          @end_auth_access


                                          @auth_access('agent-admin-cons-user-activation')
                                          @if($user->is_active == 0)
                                            <li><a href="{{ url($module.'/users-activation/active/'.$user->id) }}" class="dropdown-item"><i class="fas fa-eye"></i> Active</a></li>
                                          @else
                                            <li><a href="{{ url($module.'/users-activation/deactive/'.$user->id) }}" class="dropdown-item"><i class="fas fa-eye-slash"></i> De-Active</a></li>
                                          @endif
                                          @end_auth_access


                                          @auth_access('agent-admin-cons-user-delete')
                                        <li class="padding-left-6">
                                            {{ Form::open(['route'=>['agent.users.destroy',[$module,$user->id]],'method'=>'DELETE','style'=>'display:inline']) }}
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    {{ Form::open(['route'=>['agent.users.assignRole',$module],'method'=>'POST']) }}
